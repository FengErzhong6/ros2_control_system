...
class ManagedLaunchSession:
    def __init__(
        ...
        *,
        ...
        process_matcher: Callable[[str, list[str]], bool] | None = None,
        logger=None,
    ) -> None:
        self._launch_manager = launch_manager
        self._label = label
        self._adapter = str(adapter).strip() or "launch"
        self._launch_file_path = str(launch_file_path)
        self._launch_arguments = dict(launch_arguments or {})
        self._process_matcher = process_matcher or (lambda _name, _cmd: True)
        self._logger = logger
    ...
    def matches_process(self, process_name: str, cmd: list[str]) -> bool:
        with self._lock:
            if self._shutdown_requested or self._close_requested or self._finished_event.is_set():
                return False
...
class LaunchManager:
    DEFAULT_LAUNCH_LOG_ROOT = Path("/tmp/policy_deployment_launch_logs")
    ...
    def _event_bridge_description(self) -> LaunchDescription:
        return LaunchDescription(
            [
                RegisterEventHandler(OnProcessStart(on_start=self._handle_process_start)),
                RegisterEventHandler(OnProcessExit(on_exit=self._handle_process_exit)),
