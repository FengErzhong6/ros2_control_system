...
class RuntimeManifest:
    DEFAULT_PATH = Path("/tmp/policy_deployment_supervisor_runtime.json")
    ...
    def cleanup_stale_runtime(self) -> list[str]:
        ...
        try:
            ...
            self.delete()
            return [f"Removed unreadable policy deployment runtime manifest: {exc}"]

        old_supervisor_pid = self._coerce_positive_int(data.get("supervisor_pid"))
        if old_supervisor_pid is not None and self._pid_exists(old_supervisor_pid):
            return [
                "Policy deployment runtime manifest belongs to a still-running "
                f"supervisor (pid={old_supervisor_pid}); stale cleanup skipped."
            ]

        notes: list[str] = []
        for device_id, entry in data.get("devices", {}).items():
            notes.extend(self._cleanup_device_entry(str(device_id), entry))
    ...
    def cleanup_known_orphans(self) -> list[str]:
        ...
        for proc_entry in Path("/proc").iterdir():
            ...
            device_id = self._orphan_device_id(command)
            if not device_id:
                continue
    ...
    def register_device(
        ...
        *,
        ...
        pid = self._coerce_positive_int(metadata.get("pid"))
        pgid = self._coerce_positive_int(metadata.get("pgid"))
    ...
    def _cleanup_device_entry(self, device_id: str, entry: object) -> list[str]:
        ...
        if pgid is not None and pgid not in {os.getpid(), os.getpgrp()}:
            return self._signal_process_group(
                pgid,
                device_id=device_id,
                command=command,
                pid=pid,
        ...
        if pid is not None and pid != os.getpid():
            return self._signal_process(pid, device_id=device_id, command=command)
    ...
    def _signal_process_group(
        ...
        *,
        ...
        notes = [
            f"Cleaning stale process group for {device_id}: "
            f"pgid={pgid} command={self._command_summary(command)}"
        ]
    ...
    def _signal_process(self, pid: int, *, device_id: str, command: object) -> list[str]:
        notes = [
            f"Cleaning stale process for {device_id}: "
            f"pid={pid} command={self._command_summary(command)}"
        ]
        for sig, timeout_sec in (
            (signal.SIGINT, 8.0),
            (signal.SIGTERM, 4.0),
    ...
    def _orphan_device_id(self, command: list[str]) -> str:
        executable = Path(command[0]).name
        full_command = " ".join(command)

        if "policy_marvin_forward.launch.py" in full_command:
            return "orphaned_policy_marvin_launch"
        if "marvin_dual_joint_gui_control.launch.py" in full_command:
            return "orphaned_marvin_launch"
        if executable == "motion_server":
            return "orphaned_marvin_motion_server"
        ...
        if executable == "robot_state_publisher":
            ...
            if params_file is not None and self._params_file_contains_marvin(params_file):
        ...
        if executable == "ros2_control_node":
            for params_file in self._extract_params_files(command):
                if self._params_file_contains_marvin(params_file):
                    return "orphaned_marvin_ros2_control"
                if "marvin_dual_controllers.yaml" in str(params_file):
                    return "orphaned_marvin_ros2_control"
                if "marvin_dual_trajectory_controllers.yaml" in str(params_file):
                    return "orphaned_marvin_ros2_control"
            return ""
        return ""
    ...
    def _extract_params_file(self, command: list[str]) -> Path | None:
        files = self._extract_params_files(command)
    ...
    def _params_file_contains_marvin(self, path: Path) -> bool:
        try:
            text = path.read_text(encoding="utf-8", errors="replace")
        except OSError:
            return False
        return "marvin_dual.urdf" in text or "marvin_wuji_simplified_collision" in text
    ...
    def _process_looks_orphaned(self, ppid: int | None) -> bool:
        ...
        if parent_executable == "systemd" and "--user" in parent_command:
            return True
        return False
